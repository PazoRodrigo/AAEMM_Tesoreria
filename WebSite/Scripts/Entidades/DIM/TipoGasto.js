var _Lista_TipoGasto;

class TipoGasto extends DBE {
    constructor() {
        super();
        this.IdEntidad = 0;
        this.Nombre = '';
    ...
    // ABM
    async Alta() {
        await this.ValidarCamposTipoGasto();
        ...
        try {
            ...
            let id = await ejecutarAsync(urlWsTipoGasto + "/Alta", data);
            if (id !== undefined)
                this.IdEntidad = id;
            _Lista_TipoGasto.push(this);
            return;
        } catch (e) {
            throw e;
        }
    ...
    async Modifica() {
        ...
        try {
            ...
            let id = await ejecutarAsync(urlWsTipoGasto + "/Modifica", data);
            if (id !== undefined)
                this.IdEntidad = id;
            let buscados = $.grep(_Lista_TipoGasto, function (entidad, index) {
                return entidad.IdEntidad !== id;
            });
            _Lista_TipoGasto = buscados;
            this.IdEstado = 0;
            _Lista_TipoGasto.push(this);
            return;
        } catch (e) {
    ...
    async Baja() {
        try {
            ...
            let id = await ejecutarAsync(urlWsTipoGasto + "/Baja", data);
            if (id !== undefined)
                this.IdEntidad = id;
            let buscados = $.grep(_Lista_TipoGasto, function (entidad, index) {
                return entidad.IdEntidad !== id;
    ...
    // Todos
    static async Todos() {
        if (_Lista_TipoGasto === undefined) {
            _Lista_TipoGasto = await TipoGasto.TraerTodas();
        }
        return _Lista_TipoGasto;
    }

    // Traer
    static async TraerUno(IdEntidad) {
        _Lista_TipoGasto = await TipoGasto.TraerTodos();
        let buscado = $.grep(_Lista_TipoGasto, function (entidad, index) {
    ...
    static async TraerTodas() {
        let lista = await ejecutarAsync(urlWsTipoGasto + "/TraerTodos");
        _Lista_TipoGasto = [];
        let result = [];
        if (lista.length > 0) {
            $.each(lista, function (key, value) {
                result.push(LlenarEntidadTipoGasto(value));
            });
        }
        _Lista_TipoGasto = result;
        return _Lista_TipoGasto;
    }
    // Otros
    ...
    static async ArmarGrilla(lista, div, eventoSeleccion, eventoEliminar, estilo) {
        ...
        if (lista.length > 0) {
            ...
            for (let item of lista) {
                ...
                let aItem = '<a href="#" class="mibtn-seleccionTipoGasto" data-Evento="' + eventoSeleccion + '" data-Id="' + item.IdEntidad + '">' + item.Nombre + '</a>';
                let aEliminar = '<a href="#" class="mibtn-EliminarTipoGasto" data-Evento="' + eventoEliminar + '" data-Id="' + item.IdEntidad + '"><span class="icon-bin"></span></a>';
    ...
    static async ArmarCombo(lista, div, selector, evento, ventana, Cbo) {
        ...
        $(lista).each(function () {
            cbo += '<li><a href="#" class="mibtn-seleccionTipoGasto" data-Id="' + this.IdEntidad + '" data-Nombre="' + this.Nombre + '" data-Evento="' + evento + '" > ' + this.Nombre + '</a></li>';
        });
        cbo += '</ul>';
        cbo += '</div>';
        return $('#' + div + '').html(cbo);
...
function LlenarEntidadTipoGasto(entidad) {
    let Res = new TipoGasto;
...
$('body').on('click', ".mibtn-seleccionTipoGasto", async function () {
    try {
        $this = $(this);
        let buscado = $.grep(_Lista_TipoGasto, function (entidad, index) {
...
$('body').on('click', ".mibtn-EliminarTipoGasto", async function () {
    try {
        $this = $(this);
        let buscado = $.grep(_Lista_TipoGasto, function (entidad, index) {
            return entidad.IdEntidad === parseInt($this.attr("data-Id"));
