var _Lista_TipoContacto;

class TipoContacto extends DBE {
    constructor() {
        super();
        this.IdEntidad = 0;
        this.Nombre = '';
    ...
    // ABM
    async Alta() {
        await this.ValidarCamposTipoContacto();
        ...
        try {
            ...
            let id = await ejecutarAsync(urlWsTipoContacto + "/Alta", data);
            if (id !== undefined)
                this.IdEntidad = id;
            _Lista_TipoContacto.push(this);
            return;
        } catch (e) {
            throw e;
        }
    ...
    async Modifica() {
        ...
        try {
            ...
            let id = await ejecutarAsync(urlWsTipoContacto + "/Modifica", data);
            if (id !== undefined)
                this.IdEntidad = id;
            let buscados = $.grep(_Lista_TipoContacto, function (entidad, index) {
                return entidad.IdEntidad !== id;
            });
            _Lista_TipoContacto = buscados;
            this.IdEstado = 0;
            _Lista_TipoContacto.push(this);
            return;
        } catch (e) {
    ...
    async Baja() {
        try {
            ...
            let id = await ejecutarAsync(urlWsTipoContacto + "/Baja", data);
            if (id !== undefined)
                this.IdEntidad = id;
            let buscados = $.grep(_Lista_TipoContacto, function (entidad, index) {
                return entidad.IdEntidad !== id;
    ...
    async ValidarCamposTipoContacto() {
        let sError = '';
    ...
    // Todos
    static async Todos() {
        if (_Lista_TipoContacto === undefined) {
            _Lista_TipoContacto = await TipoContacto.TraerTodas();
        }
        return _Lista_TipoContacto;
    }

    // Traer
    static async TraerUno(IdEntidad) {
        _Lista_TipoContacto = await TipoContacto.TraerTodos();
        let buscado = $.grep(_Lista_TipoContacto, function (entidad, index) {
    ...
    static async TraerTodos() {
        return await TipoContacto.Todos();
    }
    ...
    static async TraerTodas() {
        let lista = await ejecutarAsync(urlWsTipoContacto + "/TraerTodos");
        _Lista_TipoContacto = [];
        let result = [];
        if (lista.length > 0) {
            $.each(lista, function (key, value) {
                result.push(LlenarEntidadTipoContacto(value));
            });
        }
        _Lista_TipoContacto = result;
        return _Lista_TipoContacto;
    }
    // Otros
    ...
    static async ArmarGrilla(lista, div, eventoSeleccion, eventoEliminar, estilo) {
        ...
        if (lista.length > 0) {
            ...
            for (let item of lista) {
                ...
                let aItem = '<a href="#" class="mibtn-seleccionTipoContacto" data-Evento="' + eventoSeleccion + '" data-Id="' + item.IdEntidad + '">' + item.Nombre + '</a>';
                let aEliminar = '<a href="#" class="mibtn-EliminarTipoContacto" data-Evento="' + eventoEliminar + '" data-Id="' + item.IdEntidad + '"><span class="icon-bin"></span></a>';
...
function LlenarEntidadTipoContacto(entidad) {
    let Res = new TipoContacto;
...
$('body').on('click', ".mibtn-seleccionTipoContacto", async function () {
    try {
        $this = $(this);
        let buscado = $.grep(_Lista_TipoContacto, function (entidad, index) {
...
$('body').on('click', ".mibtn-EliminarTipoContacto", async function () {
    try {
        $this = $(this);
        let buscado = $.grep(_Lista_TipoContacto, function (entidad, index) {
            return entidad.IdEntidad === parseInt($this.attr("data-Id"));
