...
Namespace Entidad
    Public Class TipoPago
        ...
        Private Shared _Todos As List(Of TipoPago)
        Public Shared Property Todos() As List(Of TipoPago)
            Get
                If _Todos Is Nothing Then
                    _Todos = DAL_TipoPago.TraerTodos
                End If
                Return _Todos
            End Get
            Set(ByVal value As List(Of TipoPago))
                _Todos = value
            End Set
        End Property
...
#Region " Constructores "
        ...
        Sub New(ByVal id As Integer)
            Dim objImportar As TipoPago = TraerUno(id)
            ' DBE
        ...
        End Sub
        Sub New(ByVal DtODesde As DTO.DTO_TipoPago)
            ' DBE
...
#Region " Métodos Estáticos"
        ' Traer
        Public Shared Function TraerUno(ByVal Id As Integer) As TipoPago
            Dim result As TipoPago = Todos.Find(Function(x) x.IdEntidad = Id)
            If result Is Nothing Then
                Throw New Exception("No existen resultados para la búsqueda")
            End If
            Return result
        End Function
        Public Shared Function TraerTodos() As List(Of TipoPago)
            Return Todos
        End Function
        'Public Shared Function TraerUno(ByVal Id As Integer) As TipoPago
        '    Dim result As TipoPago= DAL_TipoPago.TraerUno(Id)
        '    If result Is Nothing Then
        '        Throw New Exception("No existen resultados para la búsqueda")
        '    End If
        '    Return result
        'End Function
        'Public Shared Function TraerTodos() As List(Of TipoPago)
        '    Dim result As List(Of TipoPago) = DAL_TipoPago.TraerTodos()
        '    If result Is Nothing Then
        '        Throw New Exception("No existen resultados para la búsqueda")
        '    End If
        '    Return result
...
#Region " Métodos Públicos"
        ...
        Public Sub Alta()
            ValidarAlta()
            DAL_TipoPago.Alta(Me)
        End Sub
        Public Sub Baja()
            ValidarBaja()
            DAL_TipoPago.Baja(Me)
        End Sub
        Public Sub Modifica()
            ValidarModifica()
            DAL_TipoPago.Modifica(Me)
        End Sub
        ' Otros
        Public Function ToDTO() As DTO.DTO_TipoPago
            Dim result As New DTO.DTO_TipoPago With {
                .IdEntidad = IdEntidad,
                .Nombre = Nombre,
...
#Region " Métodos Privados "
        ...
        Private Sub ValidarNoDuplicados()
            TipoPago.Refresh()
            Dim result As TipoPago = Todos.Find(Function(x) x.Nombre.ToUpper = Nombre And x.IdEntidad <> IdEntidad)
...
Namespace DataAccessLibrary
    Public Class DAL_TipoPago

#Region " Stored "
        Const storeAlta As String = "DIM.p_TipoPago_Alta"
        Const storeBaja As String = "DIM.p_TipoPago_Baja"
        Const storeModifica As String = "DIM.p_TipoPago_Modifica"
        Const storeTraerUnoXId As String = "DIM.p_TipoPago_TraerUnoXId"
        Const storeTraerTodos As String = "DIM.p_TipoPago_TraerTodos"
#End Region
#Region " Métodos Públicos "
        ' ABM
        Public Shared Sub Alta(ByVal entidad As TipoPago)
            ...
            End Using
        End Sub
        Public Shared Sub Baja(ByVal entidad As TipoPago)
        ...
        End Sub
        Public Shared Sub Modifica(ByVal entidad As TipoPago)
        ...
        Public Shared Function TraerTodos() As List(Of TipoPago)
            ...
            Dim pa As New parametrosArray
            Dim listaResult As New List(Of TipoPago)
            Using dt As DataTable = Connection.Connection.TraerDt(store, pa)
...
#End Region
#Region " Métodos Privados "
        Private Shared Function LlenarEntidad(ByVal dr As DataRow) As TipoPago
            Dim entidad As New TipoPago
            ' DBE
